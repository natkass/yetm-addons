...
class PickingBackDate(models.TransientModel):
    ...
    def change_to_backdate(self):
        for picking in self.picking_ids:
            ...
            acc_move_obj = self.env['account.move'].search([('stock_move_id', 'in', move_obj.ids)])

            # Update accounting moves
            for acc_mv in acc_move_obj:
                acc_mv.button_draft()
                acc_mv.name = False
            ...
            for move in move_obj:
                ...
                valuation_obj = self.env['stock.valuation.layer'].search([('stock_move_id', '=', move.id)])

                for val in valuation_obj:
                    _logger.info("==============val before==========%s", val.create_date)
                    self.env.cr.execute('UPDATE stock_valuation_layer SET create_date=%s WHERE id=%s', (self.date, val.id))
                    _logger.info("==============val after==========%s", val.create_date)

                move_line_obj = self.env['stock.move.line'].search([('move_id', '=', move.id)])
