		...
		if (config.show_stock_location == 'specific') {
			if (config.pos_stock_type == 'onhand') {
				$.each(prods, function (i, prd) {
					...
					if(prd.detailed_type == 'consu' && prd.bom_ids.length > 0){
						prd['bi_on_hand'] = prd.product_count;
						//console.log(prd.display_name + " -> " + prd['bi_on_hand']);
						return true;
					}
				});
			...
			if (config.pos_stock_type == 'available') {
				...
				$.each(prods, function (i, prd) {
					...
					if(prd.detailed_type == 'consu' && prd.bom_ids.length > 0){
						prd['bi_available'] = prd.product_count;
						prd['virtual_available'] = prd.product_count;
						//console.log(prd.display_name + " -> " + prd['bi_on_hand']);
						return true;
					}
				});
