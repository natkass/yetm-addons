...
class MaintenanceRequest(models.Model):
    ...
    def create_task(self):
        for rec in self:
            if not rec.custom_project_id:
                raise UserError(_("Please set Project"))
            task_vals = {
                'name': rec.name,
                'description': rec.description,
                # 'user_id': rec.requisition_employee_id.user_id.id,
                'user_ids': rec.requisition_employee_id.user_id.ids,
                # 'is_job_order': True,
        ...
        res = self.env['ir.actions.act_window']._for_xml_id('website_job_workorder_request.custom_wo_action_website_job_workorder_request')
        res['domain'] = str([('maintenance_request_id', '=', self.id)])
        return res
