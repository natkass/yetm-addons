...
patch(Order.prototype, {
    ...
    async printFiscalReceipt() {
        ...
        // Check if there are mixed discount scenarios (some manual, some automatic)
        let hasMixedDiscounts = false;
        let hasManualDiscounts = false;
        let hasAutoDiscounts = false;
        
        orderlinesFromOrder.forEach(orderline => {
            if (orderline.manually_set_discount) {
                hasManualDiscounts = true;
            } else if (orderline.discount > 0) {
                hasAutoDiscounts = true;
            }
        });

        hasMixedDiscounts = hasManualDiscounts && hasAutoDiscounts;

        // If there are mixed discounts, don't apply global discount - let each line handle its own discount
        if (hasMixedDiscounts || hasManualDiscounts) {
...
patch(Orderline.prototype, {
    setup(_defaultObj, options) {
        ...
        this.manually_set_discount = false;
        this.original_discount = 0;
        if (options.json) {
    ...
    init_from_JSON(json) {
        super.init_from_JSON(json);
        this.manually_set_discount = json.manually_set_discount || false;
        // this.set_service_charge(json.service_charge);
    },
    export_as_JSON() {
        const jsonResult = super.export_as_JSON();
        jsonResult.service_charge = this.service_charge;
        jsonResult.manually_set_discount = this.manually_set_discount;
        return jsonResult;
    },
    clone() {
        const orderlineClone = super.clone();
        orderlineClone.service_charge = this.service_charge;
        orderlineClone.manually_set_discount = this.manually_set_discount;
        ...
        if (!this.order.is_refund && (this.order.state != 'done' && this.order.state != 'paid')) {
            if (this.order.pos.config.module_pos_discount) {
                this.order.orderlines.forEach(function (line) {
                    // Only apply global discount if line doesn't have a manually set discount
                    if (!line.manually_set_discount) {
            ...
            if (this.order.get_partner() != undefined && this.order.pos.config.module_pos_discount) {
                if (self.order.pos.config.discount_pc > self.order.get_partner().discount_customer) {
                    ...
                    this.order.orderlines.forEach(function (line) {
                        // Only apply customer discount if line doesn't have a manually set discount
                        if (!line.manually_set_discount) {
    ...
    set_discount(discount) {
        // If this is the first time setting a discount or the discount changed from user interaction,
        // we consider it manually set
        if (this.discount === 0 && discount > 0) {
            this.manually_set_discount = true;
        } else if (this.manually_set_discount && discount !== this.discount) {
            // If discount was previously manually set and is being changed, keep it as manually set
            this.manually_set_discount = true;
        }
