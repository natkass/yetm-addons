...
class website_account(website_account):
    ...
    def _prepare_portal_layout_values(self):
        ...
        custom_wo_joborder_count = request.env['project.task'].sudo().search_count([('custom_wo_job_partner_id','child_of', partner.id)])
        values.update({
...
#        project_id = request.env['project.project'].sudo().search([('code', '=', post.get('project_code'))], limit=1)
    ...
    def custom_wo_my_joborders(self, page=1, date_begin=None, date_end=None, joborder=None, sortby=None, **kw):
        ...
        domain = [('custom_wo_job_partner_id','child_of', partner.id)]
    ...
    def custom_wo_joborder_print(self, custom_wo_joborder, access_token=None, **kw):
        ...
        pdf, _ = request.env['ir.actions.report'].sudo()._render_qweb_pdf('website_job_workorder_request.custom_wo_joborder_report', [custom_wo_joborder])
        pdfhttpheaders = [
